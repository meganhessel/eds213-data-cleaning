...
#snowsurvey_csv_cleaned <- read.csv(file.path(datadir_raw, snowsurvey_file), na.strings = c("-", ".", "unk", "NA", "n/a")) 


glimpse(snowsurvey_csv)

# Oh no! all the measurements are read as characters! 
```
...
```{r}
snowsurvey_csv %>%
  count(Snow_cover) %>% # Tally of every unique value 
  filter(is.na(as.numeric(Snow_cover))) # Filter for only non numeric values 


# snowsurvey_csv_cleaned %>%
#   count(Snow_cover) %>% # Tally of every unique value 
#   filter(is.na(as.numeric(Snow_cover))) # Filter for only non numeric values 
# 
# snowsurvey_csv_cleaned %>%
...
# Data cleaning
```{r}
# Cleaning data one by one to ensure data safety
...
species_long <- species_csv %>% 
  ...
  pivot_longer(cols = !c(Year, Site, Date), #Alder_Flycatcher:Yellow_rumped_Warbler, 
             names_to = "Species",
             values_to = "Presence", 
...
species_presence <- species_long %>%
  mutate(Presence = ifelse(Presence == 0, 0, 1), 
         Presence = ifelse(Presence == "..", NA, Presence))
